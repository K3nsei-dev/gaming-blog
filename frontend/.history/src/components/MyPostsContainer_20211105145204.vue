<template>
  <div class="flex column col-8">
      ...
      <div v-if="blogs" class="blogs-container">
        <h3 class="q-px-xl my-blogs-heading">My Posts</h3>
        <div v-for="blog in blogs" :key="blog.id" class="flex column blog q-px-xl">
            ...
            <div class="flex row edt-del-buttons">
                <router-link :to="{ name: 'EditBlog', params: {id: blog.id} }">Edit Blog</router-link>
                <div @click='deletePost(blog.id)'>Delete Blog</div>
            </div>
        </div>
      </div>
  </div>
...
<script>
import deleteBlog from '../composables/deletePost'
...
export default {
  ...
  methods: {
    deletePost (blogId) {
      const deleteConfirm = confirm('Are you sure you want to delete this blog?')
      if (deleteConfirm) {
        try {
          // function for delete
          deleteBlog(blogId)
          ...
          alert('Blog delete unsuccessful. Please try again.')
        }
      }
    }
  }
